...
for src in list(calc_dir.glob("*.html")):
    ...
    if 'id="calc-results"' in text and 'id="reno-legacy-actions"' not in text:
        legacy_actions = """<div id="reno-legacy-actions" class="article" style="margin:16px 0;background:#eef4f0;border:1px solid #cfe2d7;border-radius:12px"><h2>Take this estimate with you</h2><p>Copy the result before comparing supplier quotes or sending the quantity to your installer.</p><div class="tool-actions" style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" type="button" onclick="copyRenoLegacy()">Copy estimate</button><button class="btn" type="button" onclick="window.print()">Print estimate</button><button class="btn" type="button" onclick="shareRenoLegacy()">Share</button><span id="reno-legacy-status" aria-live="polite"></span></div><script>function renoLegacyText(){const title=document.querySelector("h1")?.innerText||"RenoMetric estimate";const result=document.getElementById("calc-results")?.innerText||"";return title+"\\n"+result}function copyRenoLegacy(){const status=document.getElementById("reno-legacy-status");if(!navigator.clipboard){if(status)status.textContent="Copy is unavailable in this browser.";return}navigator.clipboard.writeText(renoLegacyText()).then(()=>{if(status)status.textContent="Estimate copied."}).catch(()=>{if(status)status.textContent="Copy failed."})}function shareRenoLegacy(){const status=document.getElementById("reno-legacy-status"),text=renoLegacyText();if(navigator.share){navigator.share({title:document.querySelector("h1")?.innerText||"RenoMetric estimate",text,url:location.href}).catch(()=>{});return}if(navigator.clipboard){navigator.clipboard.writeText(text).then(()=>{if(status)status.textContent="Copied for sharing."}).catch(()=>{if(status)status.textContent="Sharing is unavailable."})}else if(status)status.textContent="Sharing is unavailable."}</script></div>"""
        text = re.sub(r'(<aside\\b[^>]*id="calc-results"[^>]*>.*?</aside>)', r'\\1' + legacy_actions, text, count=1, flags=re.S | re.I)
